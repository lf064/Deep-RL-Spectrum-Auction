...
from cont_config import ContinuousLLGAuctionConfig, get_continuous_llg_config

class ContinuousLLGAuctionEnv(gym.Env):
    """
    Continuous Local-Local-Global (LLG) Auction Environment
    
    Features:
    - Discrete valuations: Locals=$4 fixed, Global=$4 or $10 (random)
    - Continuous actions: price range [min_price, max_price] for each item
    - Action space: Box([min_price, min_price], [max_price, max_price]) for two items
    - Observation: [local1_decision, local2_decision, global_decision]
      - 0 = neutral, 1 = reject, 2 = accept
    - Reward: +1 for market clearing, -1 otherwise
    """
    ...
    def __init__(self, config: ContinuousLLGAuctionConfig):
        ...
        # Action space: continuous prices for each item
        ...
        self.current_prices = [0.0] * config.num_items
    ...
    def step(self, action):
        ...
        # Clip action to valid price range and convert to prices
        self.current_prices = [
            np.clip(float(action[i]), self.config.min_price, self.config.max_price)
...
def test_continuous_llg_environment():
    """Test the continuous LLG environment"""
    print("Testing Continuous LLG Environment")
    print("=" * 40)
    
    config = get_continuous_llg_config()
    env = ContinuousLLGAuctionEnv(config)
    ...
    for episode in range(3):
        ...
        test_actions = [
            np.array([5.0, 5.0], dtype=np.float32),  # $5.0+$5.0=$10.0
            np.array([4.0, 4.0], dtype=np.float32),  # $4.0+$4.0=$8.0
            np.array([6.5, 6.5], dtype=np.float32),  # $6.5+$6.5=$13.0
            np.array([2.5, 2.5], dtype=np.float32),  # $2.5+$2.5=$5.0
        ]
        ...
        for action in test_actions:
            ...
            total_price = sum(action)
            print(f"    Action [{action[0]:.1f}, {action[1]:.1f}] → Prices ${action[0]:.1f}+${action[1]:.1f}=${total_price:.1f}: "
                  f"Success={info['successful_allocation']}, Type={info['allocation_type']}")

if __name__ == "__main__":
    test_continuous_llg_environment()